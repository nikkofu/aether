...
func TestLLMSkill_Execute_CustomTemplate(t *testing.T) {
	...
	skill := NewLLMSkill("test-skill", mockAdapter, nil, nil, nil, "Default template")

	input := map[string]any{
		"custom_template": "Custom: {{.val}}",
		"prompt_data": map[string]any{
			"val": "override",
		},
	}
...
func TestLLMSkill_Execute_ErrorCase(t *testing.T) {
	t.Run("AdapterError", func(t *testing.T) {
		...
		skill := NewLLMSkill("error-skill", mockAdapter, nil, nil, nil, "template")
		...
		capErr, ok := err.(*capability.CapabilityError)
		if !ok {
			t.Errorf("期望得到 *capability.CapabilityError, 实际得到: %T", err)
		} else if capErr.Type != capability.ErrorTypeExecution {
			t.Errorf("期望错误类型为 EXECUTION_FAILURE, 实际得到: %s", capErr.Type)
		}
	})
	...
	t.Run("RenderingError", func(t *testing.T) {
		...
		skill := NewLLMSkill("render-error", mockAdapter, nil, nil, nil, "{{.unknown | invalid_func}}")
