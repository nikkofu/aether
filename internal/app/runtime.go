...
func (r *Runtime) TaskStore() taskdomain.Store                  { return r.taskStore }
func (r *Runtime) TaskService() *taskusecase.Service            { return r.taskService }
...
func NewRuntime(cfg *config.Config) *Runtime {
	...
	logFormat := "console"
	if os.Getenv("AETHER_LOG_FORMAT") == "json" {
		logFormat = "json"
	}
...
func (a *issueWatcherAgent) Handle(ctx context.Context, msg agent.Message) ([]agent.Message, error) {
	if msg.Type == "system.alert" {
		orgID := "default"
		if m, ok := msg.Payload["org_id"].(string); ok {
			orgID = m
		}
...
func (r *Runtime) InitAgent(role string) error {
	...
	switch role {
	case "supervisor":
		a = agent.NewSupervisorAgent(role, r.tracer, r.logger)
	case "planner":
		a = agent.NewPlannerAgent(role, llm, r.tracer)
	case "coder":
		a = agent.NewCoderAgent(role, llm, r.tracer)
	case "reviewer":
		a = agent.NewReviewerAgent(role, llm, r.tracer)
	default:
